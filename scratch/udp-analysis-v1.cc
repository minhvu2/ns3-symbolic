...
//-----------------------------------------------
// Create one udpServer applications on node zero.
//
  uint16_t port1 = 8000;
...
//
// Create one UdpClient application to send UDP datagrams from node one to
// node zero.
//
  serverAddress = Address (i.GetAddress (0));
  UdpClientHelper client1 (serverAddress, port1);
  client1.SetAttribute ("MaxPackets", UintegerValue (maxPacketCount));
  client1.SetAttribute ("Interval", TimeValue (interPacketInterval));
  client1.SetAttribute ("PacketSize", UintegerValue (MaxPacketSize));
  apps1 = client1.Install (n.Get (1));
