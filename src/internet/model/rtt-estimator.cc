...
{
  //<M> change from double to uinteger		
  static TypeId tid = TypeId ("ns3::RttMeanDeviation")
    ...
    .AddAttribute ("Alpha",
                   ...
                   IntegerValue (125),
                   MakeIntegerAccessor (&RttMeanDeviation::m_alpha),
                   MakeIntegerChecker<int64_t> ())
    .AddAttribute ("Beta",
                   "Gain used in estimating the RTT variation, must be 0 <= beta <= 1",
                   IntegerValue (250),
                   MakeIntegerAccessor (&RttMeanDeviation::m_beta),
                   MakeIntegerChecker<int64_t> ())
  ;
  return tid;
}
...
//        }
      // <M>
      // SRTT = SRTT + 125/1000 (R' - SRTT)
      int64_t delta = m.ToInteger (Time::S) - m_estimatedRtt.ToInteger (Time::S);
      int64_t srtt = m_estimatedRtt.ToInteger (Time::S) + delta*m_alpha/1000;
      m_estimatedVariation = Time::FromInteger (srtt, Time::S);
      // RTTVAR = RTTVAR + beta (|SRTT - R'| - RTTVAR)
      if (delta < 0)
        {
		  delta = -delta;
		}
	  delta -= m_estimatedVariation.ToInteger (Time::S);
	  int64_t rttvar = m_estimatedVariation.ToInteger (Time::S) + delta*m_beta/1000;
	  m_estimatedVariation = Time::FromInteger (rttvar, Time::S);
      // <M>
